...
/**
 * Revamped {@link GuiPlayerTabOverlay}
 * @author VoidAlchemist
 *
 */
@Hook("net.minecraft.client.gui.GuiIngame#overlayPlayerList -> line 84"
		+ "#GuiIngame -> line 120"
		+ "#getTabList() -> line 1074")
public class GuiPlayerTabXenon extends Gui
{
    private static final Ordering<NetworkPlayerInfo> field_175252_a = Ordering.from(new GuiPlayerTabXenon.PlayerComparator());
    ...
    /** Wether or not the playerlist is currently being rendered */
    private boolean isBeingRendered;

    public GuiPlayerTabXenon(Minecraft mcIn, GuiIngame guiIngameIn)
    {
        this.mc = mcIn;
     ...
     */
    public String getPlayerName(NetworkPlayerInfo n)
    {
    	String s;
    	if (n.getDisplayName() == null)
    		s = ScorePlayerTeam.formatPlayerName(n.getPlayerTeam(), n.getGameProfile().getName());
    	else
    		s = n.getDisplayName().getFormattedText();
    	if (s.contains(Minecraft.getMinecraft().getSession().getUsername()))
    		s = XenonClient.instance.settings.customName;
        return s;
    }
